...
export async function deployAdaptersRegistry(
  ...
  console.log("Set GMX Adapter Library on Registry");
  let txResult = await adaptersRegistryContract.addAdapterAddress(
    gmxLibraryAddress
  );
  console.log("GMX Adapter Set txHash: ", txResult.hash);

  console.log("Set Uniswap Adapter on Registry");
  txResult = await adaptersRegistryContract.addAdapterAddress(
