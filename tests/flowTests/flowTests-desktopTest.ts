...
import { TEST_TIMEOUT, AMOUNT_1E18, AMOUNT_1E6 } from "../_helpers/constants";
...
const showStructsAndVariables = async () => {
  ...
  console.log(
    ...
    await usersVaultContract.afterRoundBalance()
  );  
};
  ...
  describe("UsersVault deployment Tests", function () {
    ...
    describe("WHEN trying to deploy TraderWallet contract with correct parameters", function () {
      before(async () => {
        ...
        await usdcTokenContract.mint(user1Address, AMOUNT_1E6.mul(1000));
        await usdcTokenContract.mint(user2Address, AMOUNT_1E6.mul(1000));
        await usdcTokenContract.mint(user3Address, AMOUNT_1E6.mul(1000));
        await usdcTokenContract.mint(user4Address, AMOUNT_1E6.mul(1000));
        await usdcTokenContract.mint(user5Address, AMOUNT_1E6.mul(1000));

        await usdcTokenContract
          .connect(user1)
          .approve(usersVaultContract.address, AMOUNT_1E6.mul(1000));
        await usdcTokenContract
          .connect(user2)
          .approve(usersVaultContract.address, AMOUNT_1E6.mul(1000));
        await usdcTokenContract
      ...
      describe("WHEN trying to deploy TraderWallet contract with correct parameters", function () {
        ...
        it("THEN ==> DEPOSIT ON ROUND 0 --> 1", async () => {
          await usersVaultContract
            .connect(user1)
            .userDeposit(BigNumber.from(AMOUNT_1E6.mul(10)));
          await usersVaultContract
            .connect(user1)
            .userDeposit(BigNumber.from(AMOUNT_1E6.mul(20)));
          await usersVaultContract
            .connect(user2)
            .userDeposit(BigNumber.from(AMOUNT_1E6.mul(30)));
          await usersVaultContract
            .connect(user2)
            .userDeposit(BigNumber.from(AMOUNT_1E6.mul(40)));
        ...
        it("THEN ==> FIRST rollover -->> 2", async () => {
          ...
          await dynamicValuationContract.setValuation(AMOUNT_1E18.mul(100));
          await dynamicValuationContract.setOraclePrice(AMOUNT_1E18);
